...
import { getServerSession } from "next-auth/next"; // penting
...
export async function GET(req) {
  try {
    ...
    const [borrowByCategory] = await pool.query(`
      SELECT b.genre_buku as category, COUNT(br.borrow_id) as count
      FROM borrows br
    ...
    const [recentBorrows] = await pool.query(`
      SELECT br.borrow_id, br.user_id, br.id_buku, br.borrow_date, br.due_date, br.return_date,
        br.status, b.nama_buku, u.nama as user_name
      FROM borrows br
      JOIN books b ON br.id_buku = b.id_buku
